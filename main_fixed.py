...
from placement_optimizer import PlacementOptimizer, Product
...
def optimize_product_placement(yellow_positions: List[Tuple[float, float]],
                              blue_candidates: List[Tuple[float, float]],
                              weights_file: str = None) -> Dict:
    """
    Optimize blue product placement using TSP-integrated evaluation

    Args:
        yellow_positions: List of (x,y) positions for fixed yellow products
        blue_candidates: List of candidate (x,y) positions for blue product
        weights_file: Path to visibility weights file (optional)

    Returns:
        Optimization results dictionary
    """
    print("=" * 70)
    print("PRODUCT PLACEMENT OPTIMIZATION WITH TSP INTEGRATION")
    print("=" * 70)

    # Initialize visibility calculator
    visibility_calc = VisibilityCalculator(weights_file=weights_file)

    # Create optimizer
    optimizer = PlacementOptimizer(
        yellow_products=yellow_positions,
        blue_product_candidates=blue_candidates,
        visibility_calculator=visibility_calc
    )

    # Run optimization
    results = optimizer.optimize_placement(top_k=5)
    ...
    if results['best_placement']:
        best = results['best_placement']
        print("\n🎯 BEST PLACEMENT FOUND:")
        print(f"   Position: {best.position}")
        print(f"   Visibility Score: {best.visibility_score:.2f}/100")
        print(f"   TSP Tour Cost: {best.tsp_cost:.2f}")
        print(f"   Total Score: {best.total_score:.4f}")
        print("\n📊 PERFORMANCE METRICS:")
        print(f"   Candidates Evaluated: {results['total_candidates_evaluated']}")
        print(f"   Optimization Time: {results['optimization_time_seconds']:.2f} seconds")
        print(f"   Distance Cache Hits: {results['cache_performance']['distance_cache_size']}")
        print(f"   TSP Cache Hits: {results['cache_performance']['tsp_cache_size']}")

        print("\n🔝 TOP 5 PLACEMENTS:")
        for i, candidate in enumerate(results['top_k_placements'][:5], 1):
            print(f"   {i}. Position: {candidate.position} | Score: {candidate.total_score:.4f}")

    return results


def demo_tsp_integration():
    """Demonstrate TSP integration with sample data"""
    print("\n" + "=" * 70)
    print("TSP INTEGRATION DEMO")
    print("=" * 70)

    # Sample yellow product positions (fixed)
    yellow_positions = [
        (10, 20), (30, 15), (50, 25), (15, 45), (40, 40)
    ]

    # Sample blue product candidate positions
    blue_candidates = [
        (20, 30), (35, 20), (45, 35), (25, 50), (55, 15),
        (5, 35), (60, 30), (30, 55), (50, 50), (10, 10)
    ]

    print(f"Fixed yellow products: {len(yellow_positions)}")
    print(f"Blue product candidates: {len(blue_candidates)}")

    # Run optimization
    results = optimize_product_placement(yellow_positions, blue_candidates)

    # Print completion summary
    summary = f"""
🎉 TSP INTEGRATION COMPLETE

✅ Key Features Implemented:
• TSP solver with nearest neighbor approximation
• Distance and TSP solution caching for performance
• Multi-start TSP solving for better approximations
• Integrated visibility scoring with TSP cost evaluation
• Comprehensive placement optimization with ranking

⚡ Performance Optimizations:
• Distance caching: {results['cache_performance']['distance_cache_size']} calculations cached
• TSP caching: {results['cache_performance']['tsp_cache_size']} solutions cached
• Evaluation time: {results['optimization_time_seconds']} seconds for {len(blue_candidates)} candidates

📈 Results Summary:
• Best position found at {results['best_placement'].position if results['best_placement'] else 'N/A'}
• Average TSP cost: {results['average_tsp_cost']}
• Average visibility score: {results['average_visibility_score']}

The optimizer now properly evaluates each candidate blue product placement by computing
the complete TSP tour cost including the product at that location, ensuring optimal
shopping path efficiency while maintaining product visibility.
"""
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("=" * 70)
        print("RETAIL VISIBILITY & PLACEMENT OPTIMIZER")
        print("=" * 70)
        print("\nUsage:")
        print("  1. Train model:      python main.py train <csv_path> [metric]")
        print("  2. Test item:        python main.py test")
        print("  3. Optimize placement: python main.py optimize")
        print("\nExamples:")
    ...
    if command == 'train':
        ...
        print("Use 'train', 'test', or 'optimize'")
        sys.exit(1)