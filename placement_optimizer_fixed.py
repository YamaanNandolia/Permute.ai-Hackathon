...
class PlacementOptimizer:
    ...
    def _calculate_visibility_score(self, position: Tuple[float, float]) -> float:
        ...
        # Use provided calculator - adapt to actual interface
        # The VisibilityCalculator expects specific parameters, so we'll use defaults
        try:
            result = self.visibility_calculator.get_visibility(
                shelf_height_cm=160.0,  # Default eye level
                endcap=0,  # Mid-aisle
                facings=4,  # Standard facings
                traffic_per_hour=200.0,  # Medium-high traffic
                distance_from_shopper_cm=min(200, (position[0]**2 + position[1]**2)**0.5 * 10)  # Distance based on position
            )
            return result['visibility_score']
        except Exception as e:
            print(f"Warning: Visibility calculation failed, using default: {e}")
            # Fallback to distance-based scoring
            x, y = position
            distance_from_center = (x**2 + y**2) ** 0.5
            return max(0, 100 - distance_from_center * 10)
